...
public class WindowPageTestActivity extends BaseActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_window_page_test);
        mWhiteboardView = findViewById(R.id.white);

        findViewById(R.id.pageInfo).setOnClickListener(v -> {
            logRoomInfo("PageState:" + mRoom.getRoomState().getPageState());
        });
        findViewById(R.id.addPage).setOnClickListener(v -> {
            mRoom.addPage();
        });
        findViewById(R.id.removePage).setOnClickListener(v -> {
            mRoom.removePage(1, new Promise<Boolean>() {
                @Override
